...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _normalColor = [UIColor colorWithRed:0.725 green:0.725 blue:0.725 alpha:1];
        _selectedColor = [UIColor blueColor];//kColor_themeWithAlpha(1);
...
- (void)setButtonTitles:(NSArray*)titles canMove:(BOOL)canmove index:(NSInteger)index{
    ...
    if (canmove) {
         ...
         */
        for (int i = 0; i< titles.count; i++) {
            LSBarItem* item = [[LSBarItem alloc] initWithTitle:titles[i] itemWidth:kSCREEN_WIDTH/titles.count];
            item.titleLabel.textColor = _normalColor;
            item.tag = 100+i;
            [rootScrollView addSubview:item];
